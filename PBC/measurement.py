...
def measure_Sz(N,Sz_sys,Sz_env,O_sysnew,O_envnew,B,C,num, J1, J2):
    ...
    for j in range(num): 
        ...
        for i in range(N):              # i means measuring the i+1 site，
            ...
            if i < N/2 - 1:             
                ...
                Sz[i] = state.T @ np.kron(np.kron(np.kron(Sz_matrix, Is), Is), I_env) @ state
        ...
        # np.savetxt('measurement_Sz.txt', [sum(Sz)])
        if abs(sum(Sz))<1e-10:
            with open('measurement_Sz.txt', 'a') as f:  # 'a'
                f.write(f'{N:.0f},{J1:.4f},{J2:.4f},{sum(Sz):.8f}, {C[j]:.8f}\n')  # 
